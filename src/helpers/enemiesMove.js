...
const enemiesMove = ( floor, [herox,heroy]) => {
    ...
    const enemyMoves = enemyCoordsArr.map(coordpair => {
    	...
    	if(herox < x){
    		if(floor[x-1][y].contains==="none"){moves.push([x-1, y])}
    		if(heroy < y){
    			if(floor[x][y-1].contains==="none"){moves.push([x, y-1])}
    			if(floor[x+1][y].contains==="none"){moves.push([x+1, y])}
    			if(floor[x][y+1].contains==="none"){moves.push([x, y+1])}
    		}
    	}
    ...
    enemyMoves.forEach(({coordpair, moves}) => {
    	...
    	if(moves[0]){
    		if(floor[moves[0][0]][moves[0][1]].contains === "none"){
    			const isExplored = floor[moves[0][0]][moves[0][1]].explored ? true : false
    			const isExplored2 = floor[coordpair[0]][coordpair[1]].explored? true : false
    			floor[coordpair[0]][coordpair[1]]={...emptySpace, explored:isExplored2}
    			floor[moves[0][0]][moves[0][1]] = {...tempEnemy, explored:isExplored}
    			console.log(` coords at ${coordpair[0]} ${coordpair[1]} are visible: ${floor[coordpair[0]][coordpair[1]].explored}`)
    			return
    		}
    	}
    	if(moves[1]){
    		if(floor[moves[1][0]][moves[1][1]].contains === "none"){
    			const isExplored = floor[moves[1][0]][moves[1][1]].explored ? true : false
    			const isExplored2 = floor[coordpair[0]][coordpair[1]].explored? true : false
    			floor[coordpair[0]][coordpair[1]]={...emptySpace, explored:isExplored2}
    			floor[moves[1][0]][moves[1][1]] = {...tempEnemy, explored:isExplored}
    			console.log(` coords at ${coordpair[0]} ${coordpair[1]} are visible: ${floor[coordpair[0]][coordpair[1]].explored}`)
    			return
    		}
    	}
    	...
    	if(moves[2]){
    		if(floor[moves[2][0]][moves[2][1]].contains === "none"){
    			const isExplored = floor[moves[2][0]][moves[2][1]].explored ? true : false
    			const isExplored2 = floor[coordpair[0]][coordpair[1]].explored? true : false
    			floor[coordpair[0]][coordpair[1]]={...emptySpace, explored:isExplored2}
    			floor[moves[2][0]][moves[2][1]] = {...tempEnemy, explored:isExplored}
    			console.log(` coords at ${coordpair[0]} ${coordpair[1]} are visible: ${floor[coordpair[0]][coordpair[1]].explored}`)
    			return
    		}
    	}
    	...
    	if(moves[3]){
    		if(floor[moves[3][0]][moves[3][1]].contains === "none"){
    			const isExplored = floor[moves[3][0]][moves[3][1]].explored ? true : false
    			const isExplored2 = floor[coordpair[0]][coordpair[1]].explored? true : false
    			floor[coordpair[0]][coordpair[1]]={...emptySpace, explored:isExplored2}
    			floor[moves[3][0]][moves[3][1]] = {...tempEnemy, explored:isExplored}
    			console.log(` coords at ${coordpair[0]} ${coordpair[1]} are visible: ${floor[coordpair[0]][coordpair[1]].explored}`)
    			return
    		}
    	}
