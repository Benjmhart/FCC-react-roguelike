...
export default function(
  ...
  direction,
  floor,
  applyOddsBool = applyOddsBoolfunc,
  applyOddsWithinRange = applyOddsWithinRangefunc,
  applyOddsWithinArray = applyOddsWithinArrayfunc
) {
  ...
  const received = neighbors.map(neighbor => {
    const {
      ...
      atkMin,
      atkMax
    } = neighbor.enemyCellObject;
    const receivedItem = {};
    receivedItem.origin = { ...neighbor };
    receivedItem.hit = applyOddsBool(AGI * (PER * WIS) + LUK * 2);
    const dodgerate =
      trueAGI + truePER + trueWIS + trueLUK * 2 > 30
        ? 30
        : trueAGI + truePER + trueWIS + trueLUK * 2;
    receivedItem.dodge = applyOddsBool(dodgerate);
    receivedItem.crit = applyOddsBool(CHA + LUK);
    receivedItem.minDamage =
      !receivedItem.dodge && receivedItem.hit
        ? Math.ceil(atkMin + 0.8 * STR + 0.4 * AGI)
        : 0;
    receivedItem.maxDamage =
      !receivedItem.dodge && receivedItem.hit
        ? Math.ceil(atkMax + 0.8 * STR + 0.4 * AGI)
        : 0;
    receivedItem.damage =
      applyOddsWithinRange(receivedItem.minDamage, receivedItem.maxDamage) -
      totalArmor;
    if (receivedItem.damage < 0) {
      receivedItem.damage = 0;
    }
    if (receivedItem.crit) {
      receivedItem.damage *= 2;
    }
    return receivedItem;
  });
  const targets = [];
  if (character.weapon.ability !== "beserk") {
    if (
      floor[destinationCoords[0]][destinationCoords[1]].contains === "enemy"
    ) {
      targets.push({
        coords: destinationCoords,
        enemyCellObject: {
          ...floor[destinationCoords[0]][destinationCoords[1]]
        }
      });
    }
  } else {
  ...
  const dealt = targets.map(target => {
    ...
    const dodgerate =
      AGI + PER + WIS + LUK * 2 > 30 ? 30 : AGI + PER + WIS + LUK * 2;
    dealtItem.dodge = applyOddsBool(dodgerate);
    dealtItem.crit = applyOddsBool(trueCHA + trueLUK);
    dealtItem.minDamage =
      !dealtItem.dodge && dealtItem.hit
        ? Math.ceil(weapon.attackMin + 0.8 * trueSTR + 0.4 * trueAGI)
        : 0;
    dealtItem.maxDamage =
      !dealtItem.dodge && dealtItem.hit
        ? Math.ceil(weapon.attackMax + 0.8 * trueSTR + 0.4 * trueAGI)
        : 0;
    dealtItem.damage =
      applyOddsWithinRange(dealtItem.minDamage, dealtItem.maxDamage) - armor;
    ...
    if (dealtItem.kill === true) {
      ...
      if (dealtItem.willDrop) {
        ...
        if (dealtItem.getEquipment) {
          ...
          if (
            character[[dealtItem.equipDropType].rarity + 1] ===
            equipment[character.CLASS][dealtItem.equipDropType].length
          ) {
            dealtItem.willDrop = false;
            dealtItem.getEquipment = false;
          } else {
            const equipDrop =
              equipment[character.CLASS][dealtItem.equipDropType][
                character[dealtItem.equipDropType].rarity + 1
              ];
  ...
  responseObject.death =
    character.HP <= responseObject.totalDamage ? true : false;
  //if one of the targets was a boss and it died, add a win boolean
  responseObject.win = dealt.reduce((bool, item) => {
    if (item.target.enemyCellObject.boss && item.kill) {
      return true;
    }
    return bool;
  }, false);
  ...
  responseObject.totalEXPGain = dealt.reduce((total, item) => {
    if (item.kill) {
      return total + item.target.enemyCellObject.EXP;
    }
    return total;
  }, 0);
  ...
  if (received.length === 0 && dealt.length === 0) {
    return {};
  }
