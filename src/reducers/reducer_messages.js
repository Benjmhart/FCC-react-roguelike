    ...
    case CHAR_MOVE: {
      ...
      if (pl.success === false) {
        ...
        if(pl.combat===true){
          ...
          const receivedArr = pl.combatDetails.received.reduce((Arr, recItem, index) => {
            if(recItem.damage > 0) {Arr.push(index)}
            return Arr
          }, [])
          if(receivedArr.length > 1){
            ...
            const name = pl.combatDetails.received[receivedArr[0]].origin.enemyCellObject.name
            const [ coordx, coordy ] = pl.combatDetails.received[receivedArr[0]].origin.coords
            const damage = pl.combatDetails.received[receivedArr[0]].damage
            if(!pl.combatDetails.received[receivedArr[0]].hit) {newState.push(`the ${name} at [${coordx},${coordy}] swung, but missed!`)}
            if(pl.combatDetails.received[receivedArr[0]].dodge) {newState.push(`the ${name} at [${coordx},${coordy}] swung, but you dodged!`)}
            if(pl.combatDetails.received[receivedArr[0]].damage > 0) {newState.push(`the ${name} at [${coordx},${coordy}] hit you for ${damage} damage!`)}
          }
          ...
          if(DealtArr.length > 1){
            ...
            const {damage, hit, dodge, kill} = pl.combatDetails.dealt[0];
            ...
            if(damage > 0 && !kill){newState.push(`you hit the ${name} at [${coordx},${coordy}] for ${damage} damage!`)}
            if(kill){newState.push(`you killed the ${name} at [${coordx},${coordy}]!`)
            }
          }
          const drops = []
          pl.combatDetails.dealt.forEach(dealtItem => {
            if(dealtItem.equipmentDrop){drops.push(dealtItem.equipmentDrop.name)}
