...
export default function(
  ...
  switch (action.type) {
    ...
    case CHAR_MOVE: {
      ...
      if (!pl.success) {
        if (
          pl.destinationContents.contains === "wall" ||
        ...
        if (pl.combat) {
          ...
          if (pl.combatDetails.received) {
            ...
            if (receivedArr.length > 1) {
              const totalReceieved = pl.combatDetails.received.reduce(
                (total, recItem) => {
              ...
              if (totalReceieved === 0) {
                newState.push(
                  `${
                    pl.combatDetails.received.length
                  } enemies attacked, but you dodged it all you smooth criminal`
                );
              } else {
                newState.push(
                  `you were hit by ${
                    pl.combatDetails.received.length
                  } enemies for ${totalReceieved}   damage`
                );
              }
              ...
              if (!receivedArr[0].hit) {
                newState.push(
                  `the ${name} at [${coordx}  ,${coordy}] swung, but missed!`
                );
              } else {
                if (receivedArr[0].dodge) {
                  newState.push(
                    `the ${name} at [${coordx}  ,${coordy}] swung, but you dodged!`
                  );
                }
              }
              if (receivedArr[0].crit === true && receivedArr[0].damage > 0) {
                newState.push(
                  `the ${name} at   [${coordx},${coordy}] Critically hit you for ${damage} damage!`
                );
              } else if (receivedArr[0].damage > 0) {
                newState.push(
                  `the ${name} at [${coordx},${coordy}] hit you for ${damage} damage!`
                );
              }
            }
          }
          if (pl.combatDetails.dealt.length > 0) {
            const DealtArr = pl.combatDetails.dealt.reduce(
              (Arr, dealtItem, index) => {
                if (dealtItem.damage > 0) {
                  Arr.push(index);
                }
                return Arr;
              },
            ...
            if (DealtArr.length > 1) {
              const totaldealt = pl.combatDetails.dealt.reduce(
                (total, dealtItem) => {
                  return total + dealtItem.damage;
                },
                0
              );
              const totalKills = pl.combatDetails.dealt.reduce(
                (total, dealtItem) => {
                  if (dealtItem.kill) {
                    return total + 1;
                  }
              ...
              const message = `you went beserk and dealt ${totaldealt} damage to ${
                DealtArr.length
              } enemies`;
              if (totalKills >= 1) {
            ...
            if (flash) {
              newState.push(
                `your flash went off and made the ${name} mildly uncomfortable`
              );
            }
            if (slice) {
              newState.push(`you sliced through the ${name}'s' defenses`);
            }
            if (smite) {
              newState.push(`you used superpowers to smite the ${name}`);
            }
            if (!hit) {
              newState.push(
                `you swung at the ${name} at [${coordx},${coordy}], but missed!`
              );
            }
            if (dodge) {
              newState.push(
                `you swung at the ${name} at [${coordx},${coordy}], but it dodged!`
              );
            }
            if (crit && damage > 0 && !kill) {
              newState.push(
                `you critically hit the ${name} at [${coordx},${coordy}] for ${damage} damage!`
              );
            } else if (damage > 0 && !kill) {
              newState.push(
                `you hit the ${name} at [${coordx},${coordy}] for ${damage} damage!`
              );
            }
            if (kill) {
              newState.push(`you killed the ${name} at [${coordx},${coordy}]!`);
            }

            const drops = [];
            pl.combatDetails.dealt.forEach(dealtItem => {
              if (dealtItem.getEquipment && dealtItem.equipmentDrop) {
                drops.push(
                  dealtItem.equipmentDrop[dealtItem.equipDropType].name
                );
              }
              if (dealtItem.healthDrop) {
                drops.push(dealtItem.healthDrop.name);
              }
            });
            ...
            if (pl.combatDetails.LVLup) {
              newState.push(
                "You gained a Level! You are way cooler now than you were a second ago"
              );
            }
          }
        }
